...
- “设置 → 显示”可自定义非铺满状态的窗口宽度和高度；也可以直接拖动无边框窗口边缘调整，尺寸会保存在本机。铺满时修改尺寸会在退出铺满后应用。
- 底栏“隐藏操作 UI”会先显示确认提示，可选择置顶展示或置于桌面底层，默认置顶。隐藏后操作栏和 Windows 任务栏入口会消失；置顶模式会禁用应用鼠标输入，并使用设置中的目标透明度。右键系统托盘图标可以还原操作 UI、切换层级，或直接切换太阳系与各行星系统。
- 空格暂停或继续时间，R 重置视角，S 打开设置；Ctrl + Alt + J 和托盘菜单也可打开设置。
- “设置 → 显示 → 开机自启动”可控制是否在登录 Windows 后自动启动 Jovian Desk，默认关闭。
- “设置 → 显示”中的隐藏层级和隐藏透明度会与其他用户设置一起保存在本机，重启后继续生效。
